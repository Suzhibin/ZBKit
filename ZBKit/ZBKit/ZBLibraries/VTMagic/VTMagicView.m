...
- (void)updateFrameForSubviews {
    ...
    CGFloat topY = _againstStatusBar ? STATUS_HEIGHT : 0;
  ...
  // CGFloat footerY = CGRectGetHeight(_navigationView.frame);
    NSLog(@"_footerHidden:%ld",_footerHidden);
    NSLog(@"footerY:%.2f",footerY);
    ...
    #warning 增加VTContentHighStyle 6
    if (_highStyle==VTContentHighStyleDefault) {
        CGFloat contentY = _footerHidden ? CGRectGetMaxY(_navigationView.frame):CGRectGetMaxY(_footerView.frame);
       //  CGFloat contentY = CGRectGetMaxY(_navigationView.frame);
        ...
        CGFloat contentH = size.height - 0 + (_needExtendBottom ? VTTABBAR_HEIGHT : 0);
