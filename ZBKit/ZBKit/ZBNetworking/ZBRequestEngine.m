...
- (instancetype)init {
    ...
    if (self) {
        ...
        _dataCache = [YYCache cacheWithName:@"ZBDataCache"];
    }
    return self;
}
...
- (void)sendRequest:(ZBURLRequest *)request progress:(progressBlock)progress success:(requestSuccess)success failed:(requestFailed)failed{
    if (request.methodType==ZBMethodTypePOST) {
        ...
        [[ZBRequestEngine defaultEngine] postRequest:request progress:progress success:success failed:failed];
    }else if (request.methodType==ZBMethodTypeUpload){
        
        [[ZBRequestEngine defaultEngine] uploadRequest:request progress:progress success:success failed:failed];
    }else if (request.methodType==ZBMethodTypeDownLoad){
        
        [[ZBRequestEngine defaultEngine] downloadRequest:request progress:progress success:success failed:failed];
    }else{
        
        [[ZBRequestEngine defaultEngine] getRequest:request progress:progress success:success failed:failed];
    }
}
...
#pragma mark - Upload
- (NSURLSessionTask *)uploadRequest:(ZBURLRequest *)request
...
- (NSURLSessionTask *)downloadRequest:(ZBURLRequest *)request progress:(progressBlock)progress success:(requestSuccess)success failed:(requestFailed)failed{
